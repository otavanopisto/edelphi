    ...
    try {
      if (request instanceof HttpServletRequest) {
        ...
        if (!localeCode.equals(request.getLocale().toString())) {
          ...
          Config.set(request, Config.FMT_LOCALIZATION_CONTEXT, new fi.internetix.edelphi.i18n.LocalizationContext(locale));
          servletRequest = new LocaleRequestWrapper((HttpServletRequest) request, locale);
        } else {
          Config.set(request, Config.FMT_LOCALIZATION_CONTEXT, new fi.internetix.edelphi.i18n.LocalizationContext(request.getLocale()));
        }
      }
    } finally {
